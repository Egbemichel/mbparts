...
export interface BannerCard {
    ...
    onCtaClick?: () => void;
    alt: string; // For accessibility + SEO
}
        ...
        return (
            <div
                ...
                className={`relative group ${
                    isFullWidth ? containerHeight : "h-64 md:h-80"
                } overflow-hidden ${!isFullWidth ? "flex-1" : ""}`}
                ...
                <div className="absolute inset-0 overflow-hidden">
                    <div
                        className={`w-full h-full bg-cover bg-center bg-no-repeat transition-transform duration-500 ${
                            !isFullWidth ? "group-hover:scale-110" : ""
                        }`}
              ...
              <span className="bg-primary-50 text-white text-xs font-bold font-primary px-3 py-1 rounded-full uppercase tracking-wide">
                ...
                <span className="text-primary-40 text-lg md:text-xl font-bold">
                  {banner.discount}
                        ...
                        <div className="mb-4">
                            {isFullWidth ? (
                                <h1 className="text-white font-primary font-bold leading-tight text-4xl md:text-6xl lg:text-7xl">
                                    {banner.title}
                                </h1>
                            ) : (
                                <h2 className="text-white font-primary font-bold leading-tight text-2xl md:text-3xl lg:text-4xl">
                                    {banner.title}
                                </h2>
                            )}

                            {banner.subtitle && (
                                <div className="font-primary text-white mt-2">
                                    {banner.subtitle}
                                </div>
                            )}
                        </div>
                        ...
                        {banner.description && (
                            <p
                                className={`text-gray-200 mb-6 font-semibold font-secondary-1 ${
                        ...
                        <div className="flex items-center">
                            ...
                            >
                                {banner.ctaText}
                                <ArrowRight
                                    className={`ml-2 text-primary-50 group-hover:translate-x-1 transition-transform duration-300 ${
